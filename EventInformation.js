...
//===================================================================
//＜更新情報＞
//  ver1.2.0 2026/09/07 不具合修正（非公式修正版 by ミント·アルヴァスク）
 ...
 * 
 * 非公式のver1.2.0では、ロードの際にinfo文字が表示されないバグが修正されました。
 * 本修正版で不具合が発生した場合は、下記のメールアドレスまで
 * ミント・アルヴァスク宛てにご連絡ください。
 * 連絡先：shirakawahoshiaya@gmail.com
 *
 * 利用規約：
...
_PD_EventInfomation.infoInitialize = function(eventId) {
    var sprite = this.infoSprite[eventId];
...
(function() {
    ...
    Scene_Map.prototype.createSpriteset = function() {
        ...
        // マップデータとキャラクターのスプライトが準備できた時、現在のイベントページのタブを再生成する。
        $gameMap.events().forEach(function(event) {
            if (event.page()) {
                _PD_EventInfomation.setupCommand(event);
            }
        });
...
var pd_EI_Scene_Load_onLoadSuccess = Scene_Load.prototype.onLoadSuccess;
Scene_Load.prototype.onLoadSuccess = function() {
    pd_EI_Scene_Load_onLoadSuccess.call(this);

    // 既存スプライトを削除
    for (var i in _PD_EventInfomation.infoSprite) {
        var sprite = _PD_EventInfomation.infoSprite[i];
